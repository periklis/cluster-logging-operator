...
func (lr *lokiReceiver) ApplicationLogs(timeout time.Duration) (types.Logs, error) {
	rcvName := lr.app.GetName()
	res, err := lr.tc.lokiLogs(rcvName, ProjectIndexPrefix)
	if err != nil {
		return nil, err
	}
	return types.ParseLogs(res.ToString())
}
...
func (kr *lokiReceiver) RetrieveLogs() (map[string]string, error) {
...
func (tc *E2ETestFramework) createLokiQuerier() error {
	...
	return waitForPod(tc, OpenshiftLoggingNS, loki.QuerierName, defaultRetryInterval, defaultTimeout)
}

func waitForPod(tc *E2ETestFramework, namespace, name string, retryInterval, timeout time.Duration) error {
